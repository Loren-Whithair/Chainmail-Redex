...
(define-metafunction javalite get-length : (any ...) -> number [(get-length (any_0 ...))
,(length (term (any_0 ...)))])
(define-metafunction javalite default-value : T -> v [(default-value bool)
false] [(default-value unit)
unit] [(default-value C)
null ])
(define-metafunction javalite default-value* : (T ...) -> (v ...) [(default-value* ())
()]
[(default-value* (T_0 T_1 ...))
((default-value T_0) (default-value T_1) ...)])
(define-metafunction javalite h-max : h -> number
[(h-max mt) -1]
[(h-max (h [loc -> hv]))
,(max (term loc) (term (h-max h)))])
(define-metafunction javalite h-malloc : h -> number [(h-malloc h)
,(add1 (term (h-max h)))])
 59
(define-metafunction javalite
h-malloc-n-helper : number number -> (loc ...) [(h-malloc-n-helper number_b number_c)
,(let ([z (term number_b)]) (build-list (term number_c) (lambda (y) (+ y z))))])
(define-metafunction javalite h-malloc-n : h number -> (loc ...) [(h-malloc-n h number)
(loc_0 ...)
(where ((loc_0 ...)) (h-malloc-n* h number))])
(define-metafunction javalite
internal-h-malloc-n* : number (number ...) -> (number (loc ...) ...) [(internal-h-malloc-n* number_b (number_0))
(number_t (loc_1 ...))
(where (loc_1 ...) (h-malloc-n-helper number_b number_0)) (where number_t ,(if (empty? (term (loc_1 ...)))
(term number_b)
(add1 (apply max (term (loc_1 ...))))))] [(internal-h-malloc-n* number_b (number_0 number_1 number_2 ...))
(number_t (loc_0 ...) (loc_1 ...) ...)
(where (loc_0 ...) (h-malloc-n-helper number_b number_0)) (where number_i ,(if (empty? (term (loc_0 ...)))
(term number_b)
(add1 (apply max (term (loc_0 ...))))))
(where (number_t (loc_1 ...) (internal-h-malloc-n*
(define-metafunction javalite h-malloc-n* : h number ... -> [(h-malloc-n* h number_0 ...)
...)
number_i (number_1 number_2 ...)))])
((loc ...) ...)
(( loc_0 ...) ...)
(where (number (loc_0 ...) ...) (internal-h-malloc-n* (h-malloc h) (number_0 ...)))])
(define-metafunction javalite storelike-lookup : any any -> any [(storelike-lookup mt any_0)
,(error ’storelike-loopup "~e␣not␣found␣in␣~e" (term any_0) (term mt))] [(storelike-lookup (any_0 [any_t -> any_ans]) any_t)
any_ans]
...
(define (storelike-extend <= storelike k hv) (match storelike
[’mt ‘(mt [,k -> ,hv])] [‘(,storelike [,ki -> ,hvi])
(cond
[(equal? k ki)
‘(,storelike [,ki -> ,hv])] [(<= k ki)
‘(,(storelike-extend <= storelike k hv) [,ki -> ,hvi])] [else
‘((,storelike [,ki -> ,hvi]) [,k -> ,hv])])]))
(define (storelike-extend* <= storelike extend*) (match extend*
[’() storelike]
[‘([,k -> ,hv] . ,extend*)
(storelike-extend* <= (storelike-extend <= storelike k hv) extend*)]))
(define-metafunction javalite h-lookup : h loc -> hv [(h-lookup h loc)
(storelike-lookup h loc)])
(define-metafunction javalite h-extend* : h [loc -> hv] ... -> h [(h-extend* h [loc -> hv] ...)
,(storelike-extend* <= (term h) (term ([loc -> hv] ...)))])
(define-metafunction javalite ?-lookup : ? x -> loc
[(?- lookup ? x)
(storelike-lookup ? x)])
(define-metafunction javalite ?-extend* : ? [x -> loc] ... -> ? [(?-extend* ? [x -> loc] ...)
61

,(storelike-extend* id-<= (term ?) (term ([x -> loc] ...)))])
(define-metafunction javalite restricted-field-lookup : object f -> loc [(restricted-field-lookup (C_c
loc_target
(define-metafunction javalite restrict-object : object ->
object
[f_0 loc_0] ...) ... [f_c loc_c] ...)
[f_1 loc_1] ...) ...)) ...
[(restrict-object (C_c
(C_0 (C_c (C_1 ...)
(C_0 [f_0 loc_0] ...) ... (C_t [f_t0 loc_t0] ...
[f_target loc_target]
...
(term (f_t1 ... f_1 ... ...)))))])
(define-metafunction javalite field-lookup : object f -> loc [(field-lookup object f_target)
